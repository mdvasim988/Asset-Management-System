...
import { addAsset } from '@/actions/assetops'

const Addasset = ({ onClose }) => {

    const [form, setform] = useState({ name: '', brand: '', sno: '', config: '', category: '', dop: '', status: '', condition: '' })
                ...
                <div className='min-h-[86.9vh] flex justify-center items-center bg-yellow-300  '>
                    <div className="md:w-full w-[80%] max-w-sm p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-sm sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700 my-10">
                        <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); addasset(form) }}>
                            ...
                            <button type="submit" className="mb-3 w-full text-black bg-yellow-300 hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-yellow-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-yellow-300 dark:hover:bg-yellow-400 dark:focus:ring-yellow-500">Add Asset</button>
                        </form>
                    </div>
