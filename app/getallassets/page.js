...
import { deleteAsset, getAssets } from '@/actions/assetops';
import Addasset from '@/components/assetcomponents/Addasset';
    ...
    return (
        <div className='min-h-[86.9vh] py-10 bg-yellow-300'>
            <div className="mx-auto max-w-screen-xl px-4 lg:px-12 ">
                {add && <Addasset onClose={() => { setadd(false); setafteredit(!afteredit) }} />}
                {edit && <Editasset onClose={() => { setedit(false); seteditasset(null); setafteredit(!afteredit) }} assetitem={editaaset} />}
                {/* <!-- Start coding here --> */}
                <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <h3 className='text-2xl text-center text-gray-900  dark:text-white font-bold p-3 '>Asset Details</h3>

                    {(status === "loading") &&<p className='text-white font-bold text-center'>Loading...</p>}
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                        ...
                        <div className="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                            ...
                            <div className="flex items-center space-x-3 w-full md:w-auto">
                                ...
                                <button onClick={() => setIsOpen(!isOpen)} className="w-full md:w-auto flex items-center justify-center py-2 px-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700" type="button">
                                ...
                                {isOpen && (<div className="z-10 w-48 p-3 absolute top-30 right-0.5 bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <ul className="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                        <li className="flex items-center">
                                            <input id="apple" onChange={() => handleCheckboxChange('assigned')} type="checkbox" checked={selectFilters.includes('assigned')} value="assigned" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="apple" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">assigned</label>
                                        </li>
                                        <li className="flex items-center">
                                            <input id="fitbit" onChange={() => handleCheckboxChange('not-assigned')} type="checkbox" checked={selectFilters.includes('not-assigned')} value="not-assigned" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="fitbit" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">not-assigned</label>
                                        </li>
                                        <li className="flex items-center">
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-3">D.O.P</th>
                                    <th scope="col" className="px-4 py-3">Status</th>
                                    <th scope="col" className="px-4 py-3">Condition</th>
                                    <th scope="col" className="px-4 py-3" hidden={session?.user?.role=='normal' || !session}>
                                        <span>Actions</span>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {assets.filter((item) => Object.values(item).some((value) => value.toString().toLowerCase().includes(searchTerm.toLocaleLowerCase()))).filter((item) => selectFilters.length === 0 || selectFilters.includes(item.status) || selectFilters.includes(item.category) || selectFilters.includes(item.condition)).map((item) => {
                                    return <tr key={item.sno} className="border-b dark:border-gray-700">
                                        <td className="px-4 py-3 text-gray-900 dark:text-white">{item.sno}</td>
                                        ...
                                        <td className="px-4 py-3">{item.dop}</td>
                                        <td className="px-4 py-3">{item.status}</td>
                                        <td className="px-4 py-3">{item.condition}</td>
                                        <td className="flex items-center px-4 py-3" hidden={session?.user?.role=='normal' || !session}>
                                            ...
                                            <button className="font-medium text-red-600 dark:text-red-500 hover:underline ms-3" onClick={() => handleDelete(item.sno)}>Remove</button>
                                        </td>
                                    </tr>
                                })}
                            </tbody>
