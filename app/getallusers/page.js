...
import { deleteUser, getUser } from '@/actions/userops';
    ...
    return (
        <div className='min-h-[86.9vh] py-10 bg-yellow-300'>
            <div className="mx-auto max-w-screen-xl px-4 lg:px-12 ">
                {add && <Adduser onClose={() => { setadd(false); setafteredit(!afteredit) }} />}
                {edit && <Edituser onClose={() => { setedit(false); seteditasset(null); setafteredit(!afteredit) }} useritem={editaaset} />}
                {/* <!-- Start coding here --> */}
                <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <h3 className='text-2xl text-center text-gray-900  dark:text-white font-bold p-3 '>User Details</h3>
                                ...
                                {isOpen && (<div className="z-10 w-48 p-3 absolute top-30 right-0.5 bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <ul className="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                        <li className="flex items-center">
                                            <input id="apple" onChange={() => handleCheckboxChange('admin')} type="checkbox" checked={selectFilters.includes('admin')} value="assigned" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="apple" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">admin</label>
                                        </li>
                                        <li className="flex items-center">
                                            <input id="fitbit" onChange={() => handleCheckboxChange('normal')} type="checkbox" checked={selectFilters.includes('normal')} value="not-assigned" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="fitbit" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">normal</label>
                                        </li>
                                        <li className="flex items-center">
                                            <input id="razor" onChange={() => handleCheckboxChange('moderator')} type="checkbox" checked={selectFilters.includes('moderator')} value="new" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="razor" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">moderator</label>
                                        </li>
                                        {/* <li className="flex items-center">
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-3" hidden={session?.user?.role=='normal' || session?.user?.role=='moderator'}>
                                        <span>Actions</span>
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {users.filter((item) => Object.values(item).some((value) => value.toString().toLowerCase().includes(searchTerm.toLocaleLowerCase()))).filter((item) => selectFilters.length === 0 || selectFilters.includes(item.role)).map((item) => {
                                    return <tr key={item.eid} className="border-b dark:border-gray-700">
                                        ...
                                        <td className="flex items-center px-4 py-3" hidden={session?.user?.role=='normal' || session?.user?.role=='moderator'}>
