...
import Addissue from '@/components/issuecomponents/Addissue';
import { deleteIssue, getIssue } from '@/actions/issueops';
import Editissue from '@/components/issuecomponents/Editissue';
    ...
    return (
        <div className='min-h-[86.9vh] py-10 bg-yellow-300'>
            <div className="mx-auto max-w-screen-xl px-4 lg:px-12 ">
                {add && <Addissue onClose={() => { setadd(false); setafteredit(!afteredit) }} />}
                {edit && <Editissue onClose={() => { setedit(false); seteditasset(null); setafteredit(!afteredit) }} useritem={editaaset} />}
                {/* <!-- Start coding here --> */}
                <div className="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                    <h3 className='text-2xl text-center text-gray-900  dark:text-white font-bold p-3 '>Issue Details</h3>
                                ...
                                {isOpen && (<div className="z-10 w-48 p-3 absolute top-30 right-0.5 bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <ul className="space-y-2 text-sm" aria-labelledby="filterDropdownButton">
                                        <li className="flex items-center">
                                            <input id="apple" onChange={() => handleCheckboxChange('good')} type="checkbox" checked={selectFilters.includes('good')} value="good" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="apple" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">good</label>
                                        </li>
                                        <li className="flex items-center">
                                            <input id="fitbit" onChange={() => handleCheckboxChange('bad')} type="checkbox" checked={selectFilters.includes('bad')} value="bad" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 dark:focus:ring-primary-600 dark:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                            <label forhtml="fitbit" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-100">bad</label>
                                        </li>
                                        {/* <li className="flex items-center">
                        ...
                        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" className="px-4 py-3">A-ID</th>
                                    <th scope="col" className="px-4 py-3">Issue Date</th>
                                    <th scope="col" className="px-4 py-3">Return Date</th>
                            ...
                            <tbody>
                                {users.filter((item) => Object.values(item).some((value) => value.toString().toLowerCase().includes(searchTerm.toLocaleLowerCase()))).filter((item) => selectFilters.length === 0 || selectFilters.includes(item.remarks)).map((item) => {
                                    return <tr key={item.eid} className="border-b dark:border-gray-700">
                                        ...
                                        <th scope="row" className="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{item.aid}</th>
                                        <td className="px-4 py-3">{item.idate}</td>
                                        <td className="px-4 py-3">{item.rdate}</td>
