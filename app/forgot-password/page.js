'use client'
import { editUser } from '@/actions/registerops';
...
const Forgot = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await editUser({ email : email, password : password, eid : eid, secret : secret});
        if(result) {
        ...
        <div className='min-h-[86.9vh] flex justify-center items-center bg-yellow-300'>
            <div className="my-10 md:w-full w-[80%] max-w-sm p-4 bg-gray-800 border border-gray-700 rounded-lg shadow-sm sm:p-6 md:p-8 dark:bg-gray-800 dark:border-gray-700">
                <form className="space-y-4" onSubmit={handleSubmit}>
                    <h5 className="text-2xl font-bold text-center text-white dark:text-white">Reset Password</h5>                    
                    <div>
                    ...
                    <button type="submit" className="mb-3 w-full text-black bg-yellow-300 hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-yellow-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-yellow-300 dark:hover:bg-yellow-400 dark:focus:ring-yellow-500">Reset</button>
                </form>
            </div>
